// Version 09/06/2026 TITULO DEL PC
//
//
let datosGlobales = {};          
...
//let idSerieClicReal = ""; 
let origenClicAutorizado = ""; // Puede ser PANEL o DRAWER. Controla el paso al PC.
let botonPresionadoGlobal = null; // Guarda el elemento físico del DOM que recibió el clic

// Persistencia del perfil de lectura solicitado: 'coloquial' o 'medica'
let perfilExplicacionActivo = 'coloquial'; 

const PALETA_MADRES_MX = ['#162B35', '#BA1F1F', '#00BCFF', '#F27D4B', '#008736', '#78350F', '#EBD400', '#0066FF', '#7C3AED'];
const PALETA_MADRES_AD = ['#32A584', '#34C300', '#000087', '#87005A', '#686868'];
...
function asignarColoresFijos() {
    ...
    llaves.forEach(id => {
        ...
        const saturacion = z % 2 === 0 ? 90 : 80; //default 85% - 40%
        const brillo = z % 3 === 0 ? 40 : (z % 3 === 1 ? 65 : 50);     // 45% - 85% Alterna entre tonos oscuros (40%) y claros (65%) para dar contraste visual

        
        // Sembramos los colores reales de diseño en la piel de la micción global
        miccion.colorSolidoHex = `hsl(${tono}, ${saturacion}%, ${100-brillo}%)`;
        miccion.colorPastelHex = `hsl(${tono}, ${100-saturacion}%, ${brillo}%)`;
...
function obtenerConfiguracionCromatica(tipo, idOIndice) {
    ...
    const fondoDeseleccionado = `hsl(${tono}, 40%, 85%)`;
...
function renderizarListaMicciones() {
    ...
    //MODO MICCIONES HASTA LÍNEA 293
        ...
        Object.keys(MX_Por_Dia).forEach((fechaDia, idxDia) => {
            ...
            botonMaestro.setAttribute('data-fechaoriginal', fechaDia.substring(0, 10));
            ...
            botonMaestro.onclick = function(e) {
                ...
                const cromaticaFijaMadre = obtenerConfiguracionCromatica('MADRE_MX', );

                // =========================================================================
                // NIVEL 1: CLIC SIMPLE (Toggle Promedio Grueso Sólido + Remoción de Hijos)
                // =========================================================================
                if (e.detail === 1) {  //I-MX-1Click
                	origenClicAutorizado = "M-MX";
					botonPresionadoGlobal = e.currentTarget; // Almacena el botón maestro de la lista izquierda                
//                	idSerieClicReal = idxDia; // Guarda el ID real del día que tocaste
    ...
    // MODO AUTODILATACIÓN HASTA LÍNEA 506
        ...
        Object.keys(MX_Por_Ciclo).sort((a, b) => b - a).forEach(cicloKey => {
            ...
            grupoCiclo.forEach(item => {
                ...
                const cromaticaHijoIndividual = obtenerConfiguracionCromatica('HIJO', item.id);
                datosGlobales[item.id].numeroSecuencialArchivo = zAD;
                ...
                bHijo.setAttribute('data-fondo-pastel', cromaticaHijoIndividual.pastel);
                bHijo.setAttribute('data-texto-pastel', cromaticaHijoIndividual.txtPastel);
                bHijo.setAttribute('data-borde-madre', cromaticaCiclo.borde);
...
function mostrarTarjetasCalculos(idSerie) {
    // FILTRO DE SEGURIDAD GLOBAL: CANDADO ESTILO VB6 PROPUESTO POR EL UX
    if (!origenClicAutorizado || origenClicAutorizado === "") {
        return; // Exit Sub: Bloquea las llamadas automáticas fantasmas del render
    }
    ...
    if (!tituloPC || !contenedorCalculos) return;
    contenedorCalculos.innerHTML = '';

    // Consumimos las variables de control físicas antes de vaciar el canal global
    const origenAccionReal = origenClicAutorizado;
    const botonRealDOM = botonPresionadoGlobal;

    // CANAL DE DATOS MAESTRO UNIFICADO: Guardará los idMix que heredarán los botones de cálculo
    let idMixInvolucrados = []; 
    let textoReporteUX = "Métricas urodinámicas";
    let CriterioGrupo = "";

    // 1. Buscamos el registro base como plantilla estructural para pintar las nomenclaturas
    ...
    // =========================================================================
    // EL GRAN SELECT CASE REFINADO: TU ESTRUCTURA MATRIZ DE DISEÑO
    // =========================================================================
    switch (origenAccionReal) { 
        
        // -----------------------------------------------------------------
        // CASO M-MX: DESDE EL PANEL IZQUIERDO (MADRES DE MICCIONES DIARIAS)
        // -----------------------------------------------------------------
        case "M-MX":
            let IsActive = botonRealDOM ? botonRealDOM.classList.contains('active') : false;
            CriterioGrupo = botonRealDOM ? botonRealDOM.innerText + '/2026' : "";
            
            // Censo de micciones que componen el bloque de este día específico
            let totalMiccionesDiaEncendido = Object.values(datosGlobales).filter(m => m.fecha && m.fecha.startsWith(CriterioGrupo)).length;

            if (IsActive) {
                // [1A] MadreON: Métrica de ese día completo
                Object.values(datosGlobales).forEach(m => { if(m.fecha && m.fecha.startsWith(CriterioGrupo)) idMixInvolucrados.push(m.idMix); });
                textoReporteUX = `Métrica: ${totalMiccionesDiaEncendido} micciones del día ${CriterioGrupo}`;
            } else {
                // [1B] MadreOFF
                let comadresTotales = 0;
                let comadresEncendidas = 0;
                let totalMiccionesCombinadas = 0;
                let arrayFechasComadres = [];

                document.querySelectorAll('.maestro-dia').forEach(btnMadre => {
                    comadresTotales++;
                    if (btnMadre.classList.contains('active') || btnMadre.hasAttribute('data-graficar-promedio-fecha')) {
                        comadresEncendidas++;
                        const critComadre = btnMadre.innerText + '/2026';
                        arrayFechasComadres.push(critComadre);
                        
                        // Acumulamos las micciones de todas las comadres activas
                        Object.values(datosGlobales).forEach(m => {
                            if (m.fecha && m.fecha.startsWith(critComadre)) {
                                totalMiccionesCombinadas++;
                                if (!idMixInvolucrados.includes(m.idMix)) idMixInvolucrados.push(m.idMix);
                            }
                        });
                    }
                });
                            
                switch (comadresEncendidas) {
                    case 0:
                        // [1Bno] Apagón total: última micción registrada del archivo de texto
                        const llaves = Object.keys(datosGlobales);
                        if (llaves.length > 0) {
                            const ultM = datosGlobales[llaves[llaves.length - 1]];
                            idMixInvolucrados = [ultM.idMix];
                            // Extraemos la fecha completa del PHP
                            const [fStr, hStr] = ultM.fecha.split(' ');
                            const partesF = fStr.includes('-') ? fStr.split('-').reverse() : fStr.split('/');
                            textoReporteUX = `Métrica: última micción registrada (${partesF[0]}/${partesF[1]}/${partesF[2].substring(2,4)} ${hStr.substring(0,5)})`;
                        }
                        break;
                
                    case 1:
                        // [1Bsí-1] Queda una sola madre sobreviviente seleccionada
                        textoReporteUX = `Métrica: ${totalMiccionesCombinadas} micciones del día ${arrayFechasComadres[0]}`;
                        break;
                
                    default:
                        // [1Bsí-Varios] Quedan múltiples madres encendidas (Algoritmo exquisito contiguo)
                        const fechasOrd = arrayFechasComadres.map(f => ({
                            textoCorto: f.substring(0, 5),
                            obj: new Date(f.split('/').reverse().join('/'))
                        })).sort((a,b) => a.obj - b.obj);
                        
                        const difDias = Math.ceil(Math.abs(fechasOrd[fechasOrd.length-1].obj - fechasOrd[0].obj) / (1000*60*60*24));
                        if (difDias === fechasOrd.length - 1) {
                            textoTituloUX = `Métrica: ${totalMiccionesCombinadas} micciones del ${fechasOrd[0].textoCorto} al ${fechasOrd[fechasOrd.length-1].textoCorto}`;
                        } else {
                            textoReporteUX = `Métrica: ${totalMiccionesCombinadas} micciones de ${comadresEncendidas} días seleccionados`;
                        }
                        break;
                }
            }
        ...
        // -----------------------------------------------------------------
        // CASO H-MX: DESDE EL DRAWER (HIJOS / MICCIONES INDIVIDUALES)
        // -----------------------------------------------------------------
        case "H-MX":
            // El hijo está encendido si el UX lo metió explícitamente en el arreglo de sólidos
            let hijoEstaON = hijosSeleccionadosSolidos.includes(idSerie) || (origenClicAutorizado === "H-MX" || origenClicAutorizado === "DRAWER");
            const miccionHijo = datosGlobales[idSerie] || Object.values(datosGlobales).find(m => m.idMix == idSerie);
            const fechaHijo = miccionHijo ? miccionHijo.fecha : "";
            const diaHijoClave = fechaHijo ? fechaHijo.substring(0, 10) : "";

            if (hijoEstaON) {
                // [2A] HijoON
                if (hijosSeleccionadosSolidos.length === 1) {
                    // [2Ano] Único hijo seleccionado en sólido (Dato puro sin promediar)
                    idMixInvolucrados = [miccionHijo.idMix];
                    const [fStr, hStr] = fechaHijo.split(' ');
                    const pF = fStr.includes('-') ? fStr.split('-').reverse() : fStr.split('/');
                    textoReporteUX = `Métrica: micción del ${pF[0]}/${pF[1]}/${pF[2].substring(2,4)} ${hStr.substring(0,5)}`;
                } 
                else {
                    // [2Así] Hay más hijos seleccionados en sólido en la pantalla
                    let mismoDiaTodos = hijosSeleccionadosSolidos.every(idH => datosGlobales[idH] && datosGlobales[idH].fecha && datosGlobales[idH].fecha.startsWith(diaHijoClave));

                    idMixInvolucrados = hijosSeleccionadosSolidos.map(idH => datosGlobales[idH].idMix);

                    if (mismoDiaTodos) {
                        const totalHijosFisicosDelDia = Object.values(datosGlobales).filter(m => m.fecha && m.fecha.startsWith(diaHijoClave)).length;
                        const fechaFormatoCorto = diaHijoClave.substring(0, 5) + '/' + diaHijoClave.substring(8, 10);

                        if (hijosSeleccionadosSolidos.length === totalHijosFisicosDelDia) {
                            // [2Así-sí] Todos los hijos de un mismo día ON (Igual al 1A)
                            textoReporteUX = `Métrica: ${idMixInvolucrados.length} micciones del día ${fechaFormatoCorto}`;
                        } else {
                            // [2Así-no] Varios del mismo día pero no todos
                            textoReporteUX = `Métrica: ${idMixInvolucrados.length} micciones del ${fechaFormatoCorto}`;
                        }
                    } else {
                        // Selección cruzada aditiva de múltiples días diferentes
                        textoReporteUX = `Métrica: ${idMixInvolucrados.length} micciones seleccionadas`;
                    }
                }
            } 
            else {
                // [2B] HijoOFF (Deselección o desinflado parcial)
                if (hijosSeleccionadosSolidos.length > 0) {
                    // [2Bsí] Quedan más hijos encendidos en general en el drawer
                    let hermanosActivos = hijosSeleccionadosSolidos.filter(idH => datosGlobales[idH] && datosGlobales[idH].fecha && datosGlobales[idH].fecha.startsWith(diaHijoClave));

                    if (hermanosActivos.length > 0) {
                        // [2Bsí-sí] Quedan hermanos del mismo día seleccionados (Promedia los sobrevivientes de ese día)
                        idMixInvolucrados = hermanosActivos.map(idH => datosGlobales[idH].idMix);
                        textoReporteUX = `Métrica: ${idMixInvolucrados.length} micciones del ${diaHijoClave.substring(0,5)}/${diaHijoClave.substring(8,10)}`;
                    } else {
                        // [2Bsí-no] No quedan hermanos de este día pero hay otros sólidos activos en otra madre
                        const mUltSolido = datosGlobales[hijosSeleccionadosSolidos[hijosSeleccionadosSolidos.length - 1]];
                        if (mUltSolido) {
                            idMixInvolucrados = [mUltSolido.idMix];
                            const [fStr, hStr] = mUltSolido.fecha.split(' ');
                            const pF = fStr.includes('-') ? fStr.split('-').reverse() : fStr.split('/');
                            textoReporteUX = `Métrica: micción del ${pF}/${pF}/${pF.substring(2,4)} ${hStr.substring(0,5)}`;
                        }
                    }
                } else {
                    // [2Bno] No queda ningún hijo en sólido (Todos en pastel / Desinflado masivo)
                    // Recalcula el promedio general de todas las micciones que estén en seriesSeleccionadas
                    seriesSeleccionadas.forEach(idH => { if (datosGlobales[idH] && datosGlobales[idH].idMix) idMixInvolucrados.push(datosGlobales[idH].idMix); });
                    textoTituloUX = `Métrica: ${idMixInvolucrados.length} micciones seleccionadas`;
                }
            }
            break;

        // -----------------------------------------------------------------
        // RESERVADOS PARA MODO AUTODILATACIÓN (SE ACTIVARÁN MÁS ADELANTE)
        // -----------------------------------------------------------------
        case "M-AD":
        case "H-AD":
            textoReporteUX = "Modo Autodilatación en espera...";
            break;
    }

    // Estampamos el veredicto visual de tu mapa en la cabecera física
    if (tituloPC) {
        tituloPC.textContent = textoReporteUX;
        tituloPC.setAttribute('data-idmix-involucrados', idMixInvolucrados.join(','));
    }

    // =========================================================================
    // BLOQUE 3: CONSTRUCCIÓN VISUAL DE LAS TARJETAS (APPENDCHILD)
    // =========================================================================
    if (!miccionPlantilla || !miccionPlantilla.calculos) {
        origenClicAutorizado = "";
        botonPresionadoGlobal = null;
        return;
    }
    ...
    Object.keys(miccionPlantilla.calculos).forEach(clave => {
        ...
        // Si el canal de datos unificado tiene micciones registradas, promedia sus valores en vivo
        if (idMixInvolucrados.length > 0) {
            idMixInvolucrados.forEach(mxId => {
                const mData = Object.values(datosGlobales).find(m => m.idMix == mxId);
                if (mData && mData.calculos && mData.calculos[clave] && typeof mData.calculos[clave].valor !== 'undefined') {
                    // SOLUCIÓN A3: Filtro UX. Si la micción individual evaluada no pertenece a la categoría activa
                    // seleccionada por el usuario (y no es 'todos'), el motor la ignora y salta a la siguiente.
                    if (categoriaActiva !== 'todos' && mData.calculos[clave].grupo !== categoriaActiva) {
                        return; // Salta esta micción oculta
                    }

                    sumaValoresConcepto += parseFloat(mData.calculos[clave].valor) || 0;
                    conteoReales++;
                }
            });

            if (conteoReales > 0) {
                const finalProm = sumaValoresConcepto / conteoReales;
                if (conteoReales === 1) {
                    const registroUnico = Object.values(datosGlobales).find(m => m.idMix == idMixInvolucrados);
                    if (registroUnico && registroUnico.calculos && registroUnico.calculos[clave]) {
                        valorFinalCalculado = registroUnico.calculos[clave].valor;
                    }
                } else {
                    valorFinalCalculado = parseFloat(finalProm.toFixed(3));
                }
            }
        }
    ...
    // =========================================================================
    // VACIAMOS EL CANDADO GLOBAL AL SALIR (SISTEMA PROTEGIDO)
    // =========================================================================
    origenClicAutorizado = "";
    botonPresionadoGlobal = null; 
}
...
function renderizarGraficaEstandar() {
    ...
    if (datasets.length === 0) {
        ...
        // SOLUClÓN: Si no hay curvas, la gráfica se limpia, pero antes de salirse con el return,
        // le da permiso al PC para pintar el escenario 0 del apagón total
        if (typeof mostrarTarjetasCalculos === 'function') {
            mostrarTarjetasCalculos("1");
        }
        return;
    }
    ...
    // =========================================================================
    if (typeof mostrarTarjetasCalculos === 'function') {
        // SOLUClÓN: Si el médico está en el drawer (H-MX), el ancla para las tarjetas debe ser 
        // el idSerieClicReal capturado en el impacto, evitando que el elemento [0] secuestre la identidad
        const idAnclaActivo = (origenClicAutorizado === "H-MX" && idSerieClicReal) 
                              ? idSerieClicReal 
                              : ((seriesSeleccionadas && seriesSeleccionadas.length > 0) ? seriesSeleccionadas[0] : "1");
...
/*    if (btnCerrar) {
        btnCerrar.onclick = function(e) {
            ...
            // A) Cerramos el drawer visualmente
            ventana.style.display = 'none'; 
            
            // B) Reseteamos cualquier selección de hijos individuales (Limpieza total)
            hijosSeleccionadosSolidos = [];
            
            // C) Le devolvemos la soberanía de llamadas al panel izquierdo
            origenClicAutorizado = "M-MX";
            
            // Buscamos la última madre activa en pantalla para usarla de ancla física
            const ultimaMadreActiva = document.querySelector('.maestro-dia.active');
            if (ultimaMadreActiva) {
                botonPresionadoGlobal = ultimaMadreActiva;
                const idAncla = ultimaMadreActiva.getAttribute('data-id') || "1";
                
                // D) Forzamos a Chart.js a redibujar (Pintará solo los promedios gruesos de las madres)
                if (typeof renderizarGraficaEstandar === 'function') renderizarGraficaEstandar();
                
                // E) Mostramos de inmediato los cálculos promedios de los días que sigan activos
                if (typeof mostrarTarjetasCalculos === 'function') mostrarTarjetasCalculos(idAncla);
            } else {
                // Si no hay ninguna madre activa, vaciamos el lienzo
                if (typeof renderizarGraficaEstandar === 'function') renderizarGraficaEstandar();
            }
        };
    }*/
                ...
                if (clicksClon === 1) {
                    setTimeout(() => {
                        ...
                        if (clicksClon === 1) {
                            ...
                            origenClicAutorizado = "H-MX"; 
                            botonPresionadoGlobal = clonBtn; // Guarda el objeto físico del clon
                            idSerieClicReal = idMiccionReal; // Guarda el ID real (ej: "AD_3") para la gráfica
                        ...
                        if (typeof renderizarGraficaEstandar === 'function') renderizarGraficaEstandar();
                        // === ENLACE DE ORÍGENES CON ANCLA DINÁMICA DE DESINFLADO ===
                        if (typeof mostrarTarjetasCalculos === 'function') {
                            // SOLUClÓN: Si el hijo actual se apagó (OFF), el ID de control que viaja al PC 
                            // será la última micción que haya sobrevivido activa en el arreglo de sólidos.
                            // Si el arreglo se quedó vacío, le manda el ID actual para que evalúe el desinflado masivo.
                            const idAnclaDrawerReal = hijosSeleccionadosSolidos.length > 0 
                                                      ? hijosSeleccionadosSolidos[hijosSeleccionadosSolidos.length - 1] 
                                                      : idMiccionReal;
                            
                            // Registramos la identidad real en la variable global para que la gráfica la respete
                            idSerieClicReal = idAnclaDrawerReal;
                            
                            mostrarTarjetasCalculos(idAnclaDrawerReal);
                            origenClicAutorizado = "H-MX"; 
                        }
...
function inicializarDrawerArrastrable() {
    ...
    // A) RECONECTAR EL BOTÓN DE CERRAR CON LIMPIEZA DE CAPAS (HIJOS EN OFF)
        ...
        btnCerrar.onclick = function(e) {
            ...
            // 1. Ocultamos el drawer físicamente
            ventana.style.display = 'none'; 
            
            // 2. Reseteamos los arreglos de control de hijos individuales
            hijosSeleccionadosSolidos = [];
            
            // 3. Devolvemos la rectoría de llamadas a las madres
            origenClicAutorizado = "M-MX";
            
            // Buscamos la última madre que haya quedado encendida en la lista izquierda
            const madreSobreviviente = document.querySelector('.maestro-dia.active');
            if (madreSobreviviente) {
                botonPresionadoGlobal = madreSobreviviente;
                // Forzamos a la gráfica a limpiar las curvas delgadas y dejar solo los promedios gruesos
                if (typeof renderizarGraficaEstandar === 'function') renderizarGraficaEstandar();
            } else {
                if (typeof renderizarGraficaEstandar === 'function') renderizarGraficaEstandar();
            }
